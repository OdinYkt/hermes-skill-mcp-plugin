...
class FakeSkillMcpManager:
    ...
    async def get_or_create_client(
        self, session_id: str, skill_name: str, mcp_name: str, config: dict
    ):
    ...
    async def disconnect(self, session_id: str, skill_name: str, mcp_name: str) -> None:
        key = (session_id, skill_name, mcp_name)
    ...
    def get_connected_servers(self) -> list[str]:
        return [f"{s}:{sk}:{m}" for (s, sk, m) in self._clients]
...
class TestSkillMcpSchema:
    ...
    def test_schema_parameters_type_is_object(self):
        params = SKILL_MCP_SCHEMA["parameters"]
    ...
    def test_schema_all_parameters_present(self):
...
class TestArgValidation:
    ...
    def test_mcp_name_empty_string(self):
        err = _validate_args({"skill_name": "test-skill", "mcp_name": ""})
        assert err is not None
        parsed = json.loads(err)
        assert parsed["ok"] is False
        assert parsed["error_code"] == "INVALID_ARGS"
    ...
    def test_valid_args_with_tool_name(self):
        err = _validate_args(
            {"skill_name": "sk", "mcp_name": "mc", "tool_name": "t"}
        )
        assert err is None

    def test_valid_args_with_resource_name(self):
        err = _validate_args(
            {"skill_name": "sk", "mcp_name": "mc", "resource_name": "r"}
        )
        assert err is None

    def test_valid_args_with_prompt_name(self):
        err = _validate_args(
            {"skill_name": "sk", "mcp_name": "mc", "prompt_name": "p"}
        )
        assert err is None

    def test_skill_name_wrong_type(self):
        err = _validate_args({"skill_name": 123, "mcp_name": "mc", "tool_name": "t"})
        assert err is not None
        parsed = json.loads(err)
        assert parsed["ok"] is False
        assert parsed["error_code"] == "INVALID_ARGS"
...
class TestSkillDirResolution:
    ...
    def test_find_skill_dir_not_found(self, temp_skills_dir):
        result = _find_skill_dir("nonexistent-skill", [temp_skills_dir])
        assert result is None

    def test_find_skill_dir_no_skill_md(self, temp_skills_dir):
        (temp_skills_dir / "empty-dir").mkdir()
        result = _find_skill_dir("empty-dir", [temp_skills_dir])
        assert result is None
    ...
    def test_find_skill_dir_searches_multiple_dirs(self, skill_with_mcp):
        ...
        result = _find_skill_dir("unique-skill", [other_dir, skill_dir.parent])
        assert result is not None
        assert result.resolve() == skill_dir.resolve()
...
class TestHappyPath:
    ...
    async def test_valid_tool_name_returns_success(
        ...
        handler = create_handler(manager, skill_dirs=[str(skill_dir.parent)])

        # Configure the fake client's call_tool to return a result
        ...
        manager._clients[("default", "sqlite-skill", "sqlite")] = MagicMock()
        manager._clients[("default", "sqlite-skill", "sqlite")].call_tool = AsyncMock(
            side_effect=fake_call_tool
        )
        manager._clients[("default", "sqlite-skill", "sqlite")].read_resource = AsyncMock()
        manager._clients[("default", "sqlite-skill", "sqlite")].get_prompt = AsyncMock()
...
class TestSkillNotFound:
    ...
    async def test_skill_not_found_error(self, temp_skills_dir):
        manager = FakeSkillMcpManager()
        handler = create_handler(manager, skill_dirs=[str(temp_skills_dir)])
...
class TestMcpConnectFailed:
    ...
    async def test_connection_error_returns_connect_failed(
        ...
        manager = FakeSkillMcpManager()
        manager.set_connect_error(McpConnectionError("Connection refused"))
    ...
    async def test_server_exited_on_connect_returns_server_exited(
        ...
        manager = FakeSkillMcpManager()
        manager.set_connect_error(McpServerExitedError("Server exited with code 1"))
...
class TestMcpToolErrors:
    ...
    async def test_tool_not_found_error(self, skill_with_mcp):
        ...
        client = MagicMock()
        client.call_tool = AsyncMock(
            side_effect=McpToolNotFoundError("Tool 'missing' not found.")
        )
        client.read_resource = AsyncMock()
        client.get_prompt = AsyncMock()
    ...
    async def test_tool_execution_error(self, skill_with_mcp):
        ...
        client = MagicMock()
        client.call_tool = AsyncMock(
            side_effect=McpToolExecutionError("no such table: users")
        )
        client.read_resource = AsyncMock()
        client.get_prompt = AsyncMock()
    ...
    async def test_server_exited_during_call(self, skill_with_mcp):
        ...
        client = MagicMock()
        client.call_tool = AsyncMock(
            side_effect=McpServerExitedError("Server exited with code -9")
        )
        client.read_resource = AsyncMock()
        client.get_prompt = AsyncMock()
...
class TestGrepFiltering:
    ...
    @pytest.mark.asyncio
    async def test_invalid_regex_falls_back_to_unfiltered(self, skill_with_mcp):
        manager = FakeSkillMcpManager()
    ...
    @pytest.mark.asyncio
    async def test_grep_without_pattern_returns_unfiltered(self, skill_with_mcp):
        manager = FakeSkillMcpManager()
...
class TestCredentialRedaction:
    @pytest.mark.asyncio
    async def test_connection_error_redacts_credentials(self, skill_with_mcp):
        manager = FakeSkillMcpManager()
        manager.set_connect_error(
...
class TestCreateHandlerSignature:
    ...
    async def test_handler_receives_kwargs(self, skill_with_mcp):
        ...
        client.read_resource = AsyncMock()
        client.get_prompt = AsyncMock()
        manager._clients[("my-session-id", "kwargs-skill", "server")] = client
    ...
    async def test_handler_default_session_id(self, skill_with_mcp):
        ...
        client.read_resource = AsyncMock()
        client.get_prompt = AsyncMock()
        manager._clients[("default", "default-sess-skill", "server")] = client
